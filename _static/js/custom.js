...
function initializeTopNavbar() {
    ...
    navbar.innerHTML = `
        <div class="navbar-container">
            ...
            <div class="navbar-right-icons">
                <button type="button" class="social-icon nav-search-trigger" aria-label="Search" title="Search (Ctrl+K)" style="cursor:pointer;font:inherit;">
    ...
    // Wire the search icon to the theme's built-in search dialog
    const searchTrigger = navbar.querySelector('.nav-search-trigger');
    if (searchTrigger) {
        searchTrigger.addEventListener('click', function (e) {
            e.preventDefault();
            // Click the theme's own (possibly hidden) search button to open its dialog
            const themeBtn = document.querySelector('.bd-header .search-button__button, .search-button__button, .search-button');
            if (themeBtn) {
                themeBtn.click();
            } else {
                // Fallback: emulate the Ctrl+K shortcut the theme listens for
